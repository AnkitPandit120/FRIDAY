...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            "Controls Spotify playback, volume, track info, or searches and plays songs "
            "directly on Spotify on macOS. Use this whenever the user references Spotify "
            "playback, searching, playing, pausing, skipping tracks, or adjusting music volume."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action": {
                    "type": "STRING",
                    "description": "play | pause | play_pause | next | previous | get_info | search_play | volume"
                },
                "query": {
                    "type": "STRING",
                    "description": "Song name, artist, or album to search and play (required for search_play)"
                },
                "volume_level": {
                    "type": "INTEGER",
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            if API_CONFIG_PATH.exists():
                config_data = json.loads(API_CONFIG_PATH.read_text(encoding="utf-8"))
                voice = config_data.get("voice", "Charon")
        except Exception:
            pass
