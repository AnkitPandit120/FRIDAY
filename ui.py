...
CONFIG_THEME_FILE = CONFIG_DIR / "theme.json"
...
def set_theme(theme_name: str):
    ...
    if theme_name in THEMES:
        ...
        CONFIG_THEME_FILE.write_text(json.dumps({"theme": theme_name}), encoding="utf-8")

def load_theme():
    global _current_theme
    if CONFIG_THEME_FILE.exists():
        try:
            data = json.loads(CONFIG_THEME_FILE.read_text(encoding="utf-8"))
            _current_theme = data.get("theme", "blue")
        except Exception:
            pass
...
def load_voice():
    ...
    if API_FILE.exists():
        try:
            data = json.loads(API_FILE.read_text(encoding="utf-8"))
            _current_voice = data.get("voice", "Charon")
        except Exception:
            pass
...
class SettingsPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        header.setStyleSheet(f"background: {C.DARK}; border-bottom: 1px solid {C.BORDER_B};")
        header_lay = QHBoxLayout(header)
        header_lay.setContentsMargins(16, 0, 16, 0)
        
        back_btn = QPushButton("◄  BACK")
        back_btn.setFont(QFont("Courier New", 10, QFont.Weight.Bold))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        # Dashboard Page
        dashboard = QWidget()
        dashboard.setStyleSheet(f"background: {C.BG};")
        dashboard_lay = QVBoxLayout(dashboard)
        dashboard_lay.setContentsMargins(0, 0, 0, 0)
        dashboard_lay.setSpacing(0)
        
        dashboard_lay.addWidget(self._build_header())
        ...
        dashboard_lay.addLayout(body, stretch=1)
        dashboard_lay.addWidget(self._build_footer())
        
        self._stacked.addWidget(dashboard)
        
        # Settings Page
        self._settings_page = SettingsPage()
        self._settings_page.back_clicked.connect(self._show_dashboard)
    ...
    def _on_theme_changed(self, theme_key: str):
        # Refresh all panels with new theme
    ...
    def _build_header(self) -> QWidget:
        ...
        settings_btn.setFont(QFont("Courier New", 16))
